...
"""Node to save a turn to the conversation history and reset the current turn for the LangGraph agent server."""
...
def build_save_turn_node(node_name: str = "save-turn", next_node: str = "__end__"):
    async def ainvoke(
        ...
        This node takes the current conversation state, validates that the current turn and its response are set,
        adds the completed turn to the conversation history, and resets the current turn.
